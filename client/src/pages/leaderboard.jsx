...
const RankBadge = ({ rank }) => {
  ...
  return (
    <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center font-bold text-lg ${styles[rank]}`}>{rank}</div>
  );
};

// Skeleton for loading
const UserCardSkeleton = () => (
  <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex gap-6 animate-pulse">
...
// ─── UserCard — no per-item motion.div ───────────────────────────────────────
const UserCard = React.memo(({ user, rank, onClick, navigate, currentUser }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`cursor-pointer group relative flex flex-col md:flex-row items-center gap-6 p-5 rounded-2xl border transition-all hover:-translate-y-1 ${
    ...
    >
        ...
        <div className="relative">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 p-[2px]">
            <div className="w-full h-full rounded-2xl bg-gray-900 overflow-hidden flex items-center justify-center">
              {user.profile_image
                ? <img src={`${API}${user.profile_image}`} alt="pfp" loading="lazy" className="w-full h-full object-cover" />
      ...
      <div className="flex-1 text-center md:text-left">
        ...
        <div className="flex gap-2 mt-3">
          {user.badges?.map(b => (
            <span key={b} className="px-2 py-0.5 rounded-md bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-400 font-bold uppercase">{b}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid-cols-4 gap-8 px-8 border-x border-white/5 hidden lg:grid">
        {[
          { v: user.projectsBuilt ?? 0,   l: "Projects Built" },
          { v: user.modulesCompleted ?? 0, l: "Modules Done" },
          { v: user.skillsMastered ?? 0,   l: "Skills Mastered" },
          { v: user.profileViews ?? 0,     l: "Profile Views" },
        ].map(({ v, l }) => (
          <div key={l} className="text-center">
            <p className="text-xl font-bold text-white">{v}</p>
            <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">{l}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2">
        {isSelf ? (
          ...
          <>
            <button onClick={(e) => { e.stopPropagation(); navigate(`/chat/${user.id}`); }}
            ...
            <div className="relative">
              ...
              {showMenu && (
                <div onClick={(e) => e.stopPropagation()} className="absolute right-0 mt-2 w-36 bg-[#0f172a] border border-white/10 rounded-xl shadow-lg z-50 overflow-hidden">
                  <button
                    onClick={() => axios.delete(`${API}/friends/remove/${user.id}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } })
                      .then(() => { setShowMenu(false); window.location.reload(); })
          ...
          <>
            ...
            <button disabled={sent}
              onClick={(e) => {
                e.stopPropagation();
                axios.post(`${API}/friends/request/${user.id}`, {}, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } })
                  .then(() => setSent(true)).catch(() => setSent(true));
              }}
              className={`flex items-center gap-2 px-5 py-3 rounded-xl text-white transition ${sent ? "bg-gray-600 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-500"}`}>
              <UserPlus size={18} />
              {sent ? "Sent" : "Connect"}
            </button>
          </>
...
const LeaderboardPage = () => {
  ...
  const hydrateLeaderboard = useCallback((data) => {
    setCurrentUser(data.currentUser || null);
    const processed = (data?.users || []).map(u => {
      const p = u.projectsBuilt ?? 0;
      const m = u.modulesCompleted ?? 0;
      const s = u.skillsMastered ?? 0;
      return { ...u, projectsBuilt: p, modulesCompleted: m, skillsMastered: s, relevancyScore: (p * 50) + (m * 20) + (s * 30) };
    });
    setUsers(processed);
    setTotalUsers(data.totalUsers || processed.length || 0);
    setTrendingSkill(data.trendingSkill || "None");
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const cacheKey = "leaderboard:first-page";

    if (cache[cacheKey]) {
      hydrateLeaderboard(cache[cacheKey]);
      return;
    }

    axios.get(`${API}/api/leaderboard?limit=30&offset=0`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => { cache[cacheKey] = res.data; hydrateLeaderboard(res.data); })
      .catch(err => { console.error("Leaderboard fetch error:", err); setLoading(false); });
  }, [navigate, hydrateLeaderboard]);

  const filteredUsers = useMemo(() => {
    const q = search.toLowerCase();
    return users
      .filter(u => (u.name?.toLowerCase().includes(q) || u.role?.toLowerCase().includes(q)) && (activeFilter === "All" || u.role === activeFilter))
        ...
        <div className="max-w-7xl mx-auto space-y-8 pt-4">
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-yellow-500/20 rounded-lg"><Trophy className="text-yellow-500 w-6 h-6" /></div>
                <h1 className="text-4xl font-black text-white tracking-tight">Explore Talent Leaderboard</h1>
              </div>
              <p className="text-gray-400 max-w-xl">Discover top performers ranked by projects, completion rates, and mastered expertise.</p>
            </div>
            <div className="flex gap-4">
              <StatCard label="Total Talent" value={totalUsers} icon={Users} colorClass="blue" />
              <StatCard label="Trending Skill" value={trendingSkill} icon={TrendingUp} colorClass="purple" />
            </div>
          </header>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            <div className="lg:col-span-8 relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-400" size={20} />
              <input
                type="text" placeholder="Search by username, skill, or role..." value={search} onChange={(e) => setSearch(e.target.value)}
                className="w-full bg-[#0f172a] border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5"
              />
              {search && <button onClick={() => setSearch("")} className="absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-white/10 rounded-full"><X size={16} /></button>}
            </div>
            <div className="lg:col-span-4 flex gap-2">
              ...
              <button className="p-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10"><Filter size={20} /></button>
            </div>
          </section>
